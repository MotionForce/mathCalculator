<script lang="ts">
    ...
    const forme = writable<"canonique" | "generale" | "factorisee">(
        "canonique",
    );
...
>
    ...
    <div class="p-7 flex flex-col space-y-5 bg-surface-600-400 rounded-2xl">
        <h4 class="h4">
            S'il-vous-plait utiliser des points à la place des virgules.
        </h4>
        <ParamSelector
            bind:a={$string_a}
            bind:h={$string_h}
        ...
        {:else}
            <h4 class="h4">
                S'il vous plaît inscrivez les valeurs numériques pour les
